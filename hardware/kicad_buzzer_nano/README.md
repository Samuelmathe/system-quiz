...
**Pipeline utilisé** : `pcbnew.ExportSpecctraDSN()` → `freerouting.jar` (mode CLI headless, `-mp 20`) → `pcbnew.ImportSpecctraSES()` → `kicad-cli pcb drc`. 62 connexions, **0 violation de clearance, 0 élément non connecté**, keepout respecté (0 piste dedans, vérifié par script).
