...
- **C1 et les connecteurs "fils volants" (SW1, SW2, J2, J3, BT1) sont maintenant en CMS** (empreintes SMD — `C_0805`, `PinSocket_..._SMD`), par choix explicite pour un assemblage machine (PCBA) chez le fabricant plutôt qu'à souder toi-même. Même numérotation de broche que les anciennes versions traversantes, rien d'autre n'a changé côté câblage. **Restent traversants** : A1L/A1R (Nano) et J1 (nRF24) — ce sont des headers de modules enfichables, pas adaptés au CMS.
- Aperçu : `apercu_pcb.png`.

**Pipeline utilisé** : `pcbnew.ExportSpecctraDSN()` → `freerouting.jar` (mode CLI headless, `-mp 20`) → `pcbnew.ImportSpecctraSES()` → `kicad-cli pcb drc`. 69 connexions, **0 violation de clearance, 0 élément non connecté**, keepout respecté (0 piste dedans, vérifié par script).

**DRC final** : 4 avertissements, tous `lib_footprint_mismatch` (cosmétique, se corrige avec `Outils > Mettre à jour les empreintes depuis la bibliothèque`). 2 `track_dangling` initiaux (résidus d'autorouter) identifiés et supprimés après vérification de la topologie réelle — **0 track_dangling restant**.
