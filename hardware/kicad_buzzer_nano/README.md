...
- **C1 et les connecteurs "fils volants" (SW1, SW2, J2, J3) sont maintenant en CMS** (empreintes SMD — `C_0805`, `PinSocket_..._SMD`), par choix explicite pour un assemblage machine (PCBA) chez le fabricant plutôt qu'à souder toi-même. Même numérotation de broche que les anciennes versions traversantes, rien d'autre n'a changé côté câblage. **Restent traversants** : A1L/A1R (Nano), J1 (nRF24) — headers de modules enfichables, pas adaptés au CMS — et **BT1**, voir point suivant.
- **BT1 (batterie) est maintenant un vrai connecteur JST-PH** (`JST_PH_B2B-PH-K`, mâle, traversant) au lieu de simples pastilles. Les batteries LiPo standards arrivent avec une prise JST-PH **femelle** déjà sertie sur leurs fils — la batterie se branche donc directement, rien à couper ni souder sur ses fils. Volontairement resté traversant (pas de version CMS) : ce connecteur encaisse des cycles répétés de branchement/débranchement, un pad CMS se décollerait à l'usage. Broche 1 = + (fil rouge), broche 2 = − (fil noir) — convention quasi universelle sur les batteries LiPo JST-PH, mais **vérifie la polarité sur ta batterie reçue** avant de brancher.
- **Nouveau : BATLED**, connecteur 2 broches (CMS) pour le module indicateur de niveau de batterie (4 LED, 25/50/75/100% — module "1S lithium battery capacity indicator", vérifié via handsontec.com). Branché directement en parallèle sur la batterie (même net que BT1 : broche 1 = B+, broche 2 = B-, ~5mA, toujours actif, pas de bouton). Voir `../../docs/BOM_BUZZER_EQUIPE.md`.
- Aperçu : `apercu_pcb.png`.

**Pipeline utilisé** : `pcbnew.ExportSpecctraDSN()` → `freerouting.jar` (mode CLI headless, `-mp 20`) → `pcbnew.ImportSpecctraSES()` → `kicad-cli pcb drc`. 74 connexions, **0 violation de clearance, 0 élément non connecté**, keepout respecté (0 piste dedans, vérifié par script).

**DRC final** : 5 avertissements, tous `lib_footprint_mismatch` (cosmétique, se corrige avec `Outils > Mettre à jour les empreintes depuis la bibliothèque`) — **0 track_dangling** dès le premier autoroutage cette fois.
